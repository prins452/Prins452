...
function showVipIdPopup() {
  ...
  // Check if VIP ID already exists in Local Storage
  const storedVipId = localStorage.getItem('vipId');
  const vipIdElement = document.getElementById('vipId');

  if (storedVipId) {
    // If VIP ID exists, show it directly
    vipIdElement.textContent = `VIP ID: ${storedVipId}`;
  } else {
    // Fetch IP Address and Generate VIP ID
    fetch('https://api.ipify.org?format=json')
      .then((response) => response.json())
      .then((data) => {
        const generatedVipId = data.ip.replace(/\./g, ''); // Generate VIP ID
        vipIdElement.textContent = `VIP ID: ${generatedVipId}`;
        
        // Store the VIP ID in Local Storage
        localStorage.setItem('vipId', generatedVipId);
      })
      .catch(() => {
        vipIdElement.textContent = 'Error fetching IP';
      });
  }
}
